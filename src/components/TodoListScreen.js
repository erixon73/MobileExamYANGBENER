...
const TodoListScreen = props => {
    ...
    const onAddTodo = () => {
        ...
        // var patch = firebase.database().ref('id').update({
        //     status : 'finished'
        // });
                ...
                renderRow={item => {
                    ...
                    <ListItem>
                        <Left>
                         ...
                         <Button  onPress={()=>{onCompleteTodo(item.id)}} info style={{ padding: 10 }}>
                         <Text style={{ color: 'white'}}>{item.status}</Text>
                         </Button>
                         :
                         <Button onPress={()=>{onCompleteTodo(item.id)}} success style={{ padding: 10 }}>
                         <Text style={{ color: 'white'}}>{item.status}</Text>
                         </Button>     
                        }
                        </Left>
                        ...
                        <TouchableOpacity onPress={()=>{props.navigation.navigate(`TodoDetailScreen`, {
                                todo: `${item.todo}`,
                                id: item.id,
                                status: item.status,
